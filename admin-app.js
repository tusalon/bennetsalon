// admin-app.js - Bennet Salon (VERSIÓN OPTIMIZADA)
...
function AdminApp() {
    ...
    const [processingId, setProcessingId] = React.useState(null); // 🔥 Evita doble click
    ...
    // 🔥 Formato de hora 12h (de tu código)
    ...
    // 🔥 Manejo de cambio de estado (mejorado)
    const handleStatusChange = async (id, newStatus, bookingData) => {
        if (processingId === id) return; // Prevenir doble click
        
        const horaFormateada = formatTo12Hour(bookingData.hora_inicio);
        let confirmMessage = "";
        let successMessage = "";
        
        if (newStatus === "Confirmado") {
            confirmMessage = `✅ ¿Confirmar turno?\n\n👤 ${bookingData.cliente_nombre}\n📅 ${bookingData.fecha} ${horaFormateada}\n💅 ${bookingData.servicio}`;
            successMessage = "Turno confirmado";
        } else {
            confirmMessage = `❌ ¿CANCELAR turno?\n\n👤 ${bookingData.cliente_nombre}\n📅 ${bookingData.fecha} ${horaFormateada}\n💅 ${bookingData.servicio}\n\n⚠️ Se notificará al cliente.`;
        ...
        try {
            ...
            if (newStatus === "Confirmado") {
                ...
                mensaje = `❌ *TURNO CANCELADO* ❌\n\nHola *${bookingData.cliente_nombre}*, lamentamos informarte que tu turno del *${bookingData.fecha}* a las *${horaFormateada}* ha sido cancelado.\n\nPor favor, contactanos para reagendar:\n📱 +53 54438629\n\nDisculpá las molestias.`;
            }
